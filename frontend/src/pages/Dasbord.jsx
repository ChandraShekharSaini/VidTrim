...
const Dasbord = () => {
  ...
  useEffect(() => {
    ...
    const videoHistory = async (e) => {
      ...
      try {
        ...
        const response = await fetch(`https://vimtrim.onrender.com/api/compressed-video/video-history/${currentUser._id}`, {
          method: "GET",
          headers: {
  ...
  const deleteVideo = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`https://vimtrim.onrender.com/api/compressed-video/delte-video/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
          ...
          <div className='w-[50%] flex flex-row items-center justify-end pr-5 '>
            {
              currentUser ? (
                <Link to='/user/profile'>
                  <img src={currentUser.profilePicture.defaultImageUrl || currentUser.profilePicture.googleImageUrl} alt="profile" className='w-[40px] h-[40px] rounded-full object-cover cursor-pointer' />
                </Link>
              ) : ("")
            }
          </div>
